...
open class GalleryViewController: UIPageViewController, ItemControllerDelegate {
    ...
    public init(startIndex: Int, itemsDataSource: GalleryItemsDataSource, itemsDelegate: GalleryItemsDelegate? = nil, displacedViewsDataSource: GalleryDisplacedViewsDataSource? = nil, configuration: GalleryConfiguration = []) {
        ...
        for item in configuration {
            ...
            case let .hideDecorationViewsOnLaunch(hidden): decorationViewsHidden = hidden
            case let .decorationViewsFadeDuration(duration): decorationViewsFadeDuration = duration
            ...
            case let .overlayBlurStyle(style): overlayView.blurringView.effect = UIBlurEffect(style: style)
            case let .overlayBlurOpacity(opacity): overlayView.blurTargetOpacity = opacity
            case let .overlayColorOpacity(opacity): overlayView.colorTargetOpacity = opacity
            case let .blurPresentDuration(duration): overlayView.blurPresentDuration = duration
            case let .blurPresentDelay(delay): overlayView.blurPresentDelay = delay
            case let .colorPresentDuration(duration): overlayView.colorPresentDuration = duration
            case let .colorPresentDelay(delay): overlayView.colorPresentDelay = delay
            case let .blurDismissDuration(duration): overlayView.blurDismissDuration = duration
            case let .blurDismissDelay(delay): overlayView.blurDismissDelay = delay
            case let .colorDismissDuration(duration): overlayView.colorDismissDuration = duration
            case let .colorDismissDelay(delay): overlayView.colorDismissDelay = delay
            case let .continuePlayVideoOnEnd(enabled): continueNextVideoOnFinish = enabled
        ...
        setViewControllers([initialController], direction: UIPageViewController.NavigationDirection.forward, animated: false, completion: nil)
    ...
    fileprivate func layoutButton(_ button: UIButton?, layout: ButtonLayout) {
        ...
        case let .pinRight(marginTop, marginRight):
            ...
            button.frame.origin.x = view.bounds.size.width - marginRight - button.bounds.size.width
    ...
    fileprivate func layoutHeaderView() {
        ...
        case let .pinBoth(marginTop, marginLeft, marginRight):
            ...
            header.bounds.size.width = view.bounds.width - marginLeft - marginRight
            header.sizeToFit()
        ...
        case let .pinRight(marginTop, marginRight):
            ...
            header.frame.origin = CGPoint(x: view.bounds.width - marginRight - header.bounds.width, y: marginTop)
        }
    }
    ...
    fileprivate func layoutFooterView() {
        ...
        case let .center(marginBottom):
            ...
            footer.frame.origin.y = view.bounds.height - footer.bounds.height - marginBottom

        case let .pinBoth(marginBottom, marginLeft, marginRight):

            footer.autoresizingMask = [.flexibleTopMargin, .flexibleWidth]
            footer.frame.size.width = view.bounds.width - marginLeft - marginRight
            footer.sizeToFit()
            footer.frame.origin = CGPoint(x: marginLeft, y: view.bounds.height - footer.bounds.height - marginBottom)
        ...
        case let .pinRight(marginBottom, marginRight):
            ...
            footer.frame.origin = CGPoint(x: view.bounds.width - marginRight - footer.bounds.width, y: view.bounds.height - footer.bounds.height - marginBottom)
        }
    }

    fileprivate func layoutScrubber() {
        scrubber.bounds = CGRect(origin: CGPoint.zero, size: CGSize(width: view.bounds.width, height: 40))
        scrubber.center = view.boundsCenter
        scrubber.frame.origin.y = (footerView?.frame.origin.y ?? view.bounds.maxY) - scrubber.bounds.height
    }
    ...
    public func itemController(_ controller: ItemController, didSwipeToDismissWithDistanceToEdge distance: CGFloat) {
        ...
        overlayView.blurringView.alpha = 1 - distance
